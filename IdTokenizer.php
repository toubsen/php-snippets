 ...
 * HMAC to grant that an incoming tokenized ID has been generated by the 
 * ourselves and has not been changed in any way. As a result, the tokenized
 * ID can be used as an access token, e.g. for giving out links to a user,
 * that shall be granted access to a single resource without any other auth
 * checks, but make it (nearly) impossible for anyone else to guess another
 * valid token.
 * 
 ...
 * of collissions will improve, so depending on your use case you probably
 * want a number >= 64 bits up to even the complete hash size. Anything below
 * 32 bits will drastically increase the number of collissions most probably
 * bring you into severe trouble.
 * 
 * READ CAREFULLY:
 * 
 ...
 * You should be using encrypted ids + message verification with HMAC, which
 * is not implemented here, as the focus laid on the shortest possible tamper
 * proof representation, especially for small IDs.
 * 
 * Most importantly, always use different keys for each type of
